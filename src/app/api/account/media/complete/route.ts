import { NextRequest, NextResponse } from "next/server";
import { mediaCompleteSchema } from "@/schemas/media";
import { requireProfileEditor } from "@/server/auth/guards";
import { getClientIp, handle, parseJsonBody, requireSameOrigin } from "@/server/http";
import { completeMemberUpload } from "@/server/users/profile";

export const POST = handle(async (request: NextRequest, { requestId }) => {
  requireSameOrigin(request);
  const user = await requireProfileEditor();
  const input = await parseJsonBody(request, mediaCompleteSchema);
  const data = await completeMemberUpload(user.id, input.id, {
    actorId: user.id,
    requestId,
    ip: getClientIp(request),
    userAgent: request.headers.get("user-agent"),
  });
  return NextResponse.json({ data, requestId });
});
