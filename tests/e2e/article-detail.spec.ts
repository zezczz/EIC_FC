...
async function loginCaptain(request: APIRequestContext) {
...
test.describe("动态详情页", () => {
  test("中文标题发布后从列表进入详情", async ({ page, request }) => {
    const loggedIn = await loginCaptain(request);
    test.skip(!loggedIn, "开发队长账号不可用，请先执行 pnpm db:seed");

    const title = `本地验证测试新闻-${Date.now()}`;
    const createResponse = await request.post("/api/captain/articles", {
      data: {
        title,
        summary: "E2E 测试摘要",
    ...
    const publishResponse = await request.post(`/api/captain/articles/${articleId}/publish`, {
      headers: { Origin: ORIGIN },
    });
